...
public class Fase extends JPanel implements ActionListener {
	...
	public void paint(Graphics g) {
		...
		if (emJogo) {
			graficos.drawImage(pixuleco.getImagem(), pixuleco.getX(), pixuleco.getY(), this);

			List<Propina> dinheiro = pixuleco.getDinheiro();

			for (int i = 0; i < dinheiro.size(); i++) {
				Propina m = (Propina) dinheiro.get(i);
	...
	public void actionPerformed(ActionEvent e) {
		...
		List<Propina> dinheiros = pixuleco.getDinheiro();

		for (int i = 0; i < dinheiros.size(); i++) {
			Propina m = (Propina) dinheiros.get(i);
	...
	public void checarColisoes() {
		Rectangle formaNave = pixuleco.getBounds();
		...
		for (int i = 0; i < inimigos.size(); i++) {
			...
			if (formaNave.intersects(formaInimigo)) {
				pixuleco.setVisivel(false);
				tempInimigo.setVisible(false);
				emJogo = false;
			}
		}

		List<Propina> misseis = pixuleco.getDinheiro();

		for (int i = 0; i < misseis.size(); i++) {
			Propina tempDinheiro = misseis.get(i);
			formaMissel = tempDinheiro.getBounds();
	...
	private class TecladoAdapter extends KeyAdapter {
		...
		public void keyPressed(KeyEvent e) {
			...
			pixuleco.keyPressed(e);
		}

		@Override
		public void keyReleased(KeyEvent e) {
			pixuleco.keyReleased(e);
		}
	}
}
